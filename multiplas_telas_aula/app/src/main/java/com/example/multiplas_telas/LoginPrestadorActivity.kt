...
class LoginPrestadorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnLogar = findViewById<Button>(R.id.logarUserPrest)
        btnLimpar = findViewById<Button>(R.id.limpUserPrest)
        btnSair = findViewById<Button>(R.id.sairUserPrest)
        email = findViewById<EditText>(R.id.emailPrest)
        senha = findViewById<EditText>(R.id.senhaPrest)

        btnLogar.setOnClickListener {
            val valEmail = email.text.toString()
            val valSenha = senha.text.toString()

            if(valEmail == "jose@gmail.com" && valSenha == "101010"){
                Toast.makeText(this, "Logando...", Toast.LENGTH_SHORT).show()
                var intent = Intent(this@LoginPrestadorActivity, PrestadorActivity::class.java)
